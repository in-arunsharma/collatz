// V1.3d: Final sequential micro-optimizations before parallelization
//
// KEY CHANGES FROM V1.3c:
// 1. Always-inline hot kernel for better LICM/if-conversion
// 2. Restrict pointer aliasing hints for compiler optimization
// 3. Branchless even/odd with local uint64_t for common case
// 4. Progress printing off by default (--progress flag)
// 5. Enhanced compile flags (-flto, -fno-asynchronous-unwind-tables)
//
// MICRO-OPTIMIZATIONS:
// - __attribute__((always_inline)) on compute_collatz_readonly
// - const uint32_t* __restrict memo_ptr parameter
// - Hoist uint64_t lo = (uint64_t)current for parity checks
// - Optional progress reporting (CLI flag)
//
// EXPECTED PERFORMANCE: 5-10% faster than V1.3c (~2.1-2.2M nums/sec)
...
int main(int argc, char** argv) {
    ...
    std::cout << "\n=== V1.3d Results (Final Sequential Optimizations) ===\n";
