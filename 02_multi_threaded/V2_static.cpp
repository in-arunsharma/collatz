...
inline int64_t collatz_steps(__uint128_t n, __uint128_t start_range) {
    ...
    while (current != 1) {
        ...
        // Only terminate early if we drop BELOW the entire test range
        // (assumes all numbers < start_range are already verified)
...
int main() {
    ...
    const int NUM_THREADS = 12;  // 6 cores × 2 threads
    
    cout << "Collatz Multi-threaded V1 - OpenMP" << endl;
    cout << "===================================" << endl;
    cout << "Start: 2^71 = "; print_uint128(START); cout << endl;
    cout << "Range: " << COUNT << " numbers" << endl;
    cout << "Threads: " << NUM_THREADS << endl;
    cout << "Optimizations: Mod-6 + OpenMP parallel" << endl << endl;
    ...
    #pragma omp parallel
    {
        uint64_t local_total_steps = 0;
        uint64_t local_max_steps = 0;
        __uint128_t local_max_steps_number = 0;
        uint64_t local_cycles_found = 0;
        uint64_t local_numbers_tested = 0;
        
        #pragma omp for schedule(static)
        for (uint64_t i = 0; i < COUNT / 6; i++) {
            // Each iteration handles one pair of mod-6 numbers (positions at ≡1 and ≡5)
        ...
        #pragma omp critical
        {
            ...
            numbers_tested += local_numbers_tested;
            cycles_found += local_cycles_found;
            if (local_max_steps > max_steps) {
                max_steps = local_max_steps;
                max_steps_number = local_max_steps_number;
            }
        }
    }
