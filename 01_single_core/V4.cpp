...
int64_t collatz_steps(__uint128_t n) {
    ...
    __uint128_t slow = n;
    __uint128_t fast = n;
    uint64_t steps = 0;  // Total steps for slow pointer only
    ...
    while (slow != 1) {
        ...
        // If we drop below the starting value n, we've reached a value whose orbit
        // has already been verified earlier in this run
        if (slow < original) {
            return steps;
        }
        
        // Fast pointer: two steps, but don't corrupt `steps`
        if (fast != 1) {
            uint64_t tmp;
            fast = collatz_algo(fast, tmp);
        }
        if (fast != 1) {
            uint64_t tmp;
            fast = collatz_algo(fast, tmp);
        ...
        // Fast got into already-tested territory, so slow eventually will too
        if (fast < original) {
            return steps;
        }
        
        // Non-trivial cycle detected (anything except the 4-2-1 cycle)
        if (slow == fast && slow != 1) {
            return -1;
        }
    }
...
int main() {
    ...
    cout << "Collatz V4 - Early Termination Below Seed" << endl;
    cout << "==========================================" << endl;
